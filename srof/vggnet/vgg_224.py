...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1000, init_weights=True):
        ...
        self.linear1 = afnn.AfLinear(512 * 7 * 7, 4096)
        self.relu = nn.ReLU(True)
        self.drop1 = nn.Dropout()  ###### 去掉drop试试
        self.linear2 = afnn.AfLinear(4096, 4096)
        # nn.ReLU(True)
    ...
    def forward(self, x):
        ...
        x, att_0 = self.linear1(x)
        att.append(att_0)

        x = self.drop1(x)
        x, att_0 = self.linear2(x)
        att.append(att_0)
        x = self.drop2(x)
...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            conv2d = afnn.AfConv2d_bn(in_channels, v, kernel_size=3)  # , padding=1)
            if batch_norm:
