...
class GradCAM:
    def __init__(self, model: nn.Module, target_layer: str, size=(224, 224), num_cls=1000, mean=None, std=None) -> None:
        ...
        # self.model.layer4
        self.model.features[28].conv.register_forward_hook(self.__forward_hook)
        self.model.features[28].conv.register_backward_hook(self.__backward_hook)
...
# 调用函数
img = cv2.imread('../../dataset/cifar-10-batches-py/test/0_499.jpg', 1)
net = torch.load('../5_increment_10_net.pkl',map_location={'cuda:2': 'cuda:0'}).to('cpu')

grad_cam = GradCAM(net, 'features', (32, 32), 10, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
grad_cam.forward(img,  show=True, write=True)
