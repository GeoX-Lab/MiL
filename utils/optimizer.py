            ...
            for p in group['params']:
                ...
                if momentum != 0:
                    ...
                    if 'momentum_buffer' not in param_state:
                        ...
                        buf = param_state['momentum_buffer']
                        z = p.data.add(-group['lr'], d_p)  # equ 10
                        z = self.soft_thresholding(z, group['lr'] * self.gamma)
                        v = z - p.data + buf.mul_(momentum)  # equ 11
                        p.data = z + v.mul_(momentum)  # equ 12
                    # no negtive
                    p.data = torch.max(p.data, torch.zeros(len(p.data)).cuda())
                    # print("lambda", p.data)
        return loss
...
def sgd_optimizer(model, lr, weight_decay):
    params = []
